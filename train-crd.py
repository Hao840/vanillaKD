...
from utils import ImageNetInstanceSample, process_feat, setup_default_logging, TimePredictor
import models
...
# --------------------------------------  Modified ---------------------------------------
# KD parameters: DIST, DKD, and vanilla KD
parser.add_argument('--kd-loss', default='crd', type=str)
...
parser.set_defaults(no_prefetcher=True)
...
def main():
    ...
    dataset_train = ImageNetInstanceSample(root=f'{args.data_dir}/train', name=args.dataset, class_map=args.class_map,
                                           load_bytes=False, is_sample=True, k=16384)
    ...
    if args.kd_loss == 'crd':
        kd_loss_fn = CRD(feat_s_channel=config.get_pre_logit_dim(args.model),
                         feat_t_channel=config.get_pre_logit_dim(args.teacher),
                         feat_dim=128, num_data=len(dataset_train), k=16384,
                         momentum=0.5, temperature=0.07)
        requires_feat = True
    else:
        raise NotImplementedError(f'this script only supports crd loss. for other kd loss, please refer to train-kd.py')
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                requires_feat=requires_feat, lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
            ...
            if lr_scheduler is not None:
                ...
                lr_scheduler.step(epoch + 1, metrics)
...
def train_one_epoch(
        epoch, model, teacher, loader, optimizer, loss_fn, args, requires_feat=False,
    ...
    for batch_idx, (input, target, index, contrastive_index) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            with torch.no_grad():
                output_t, feat_t = teacher(teacher_input, requires_feat=requires_feat)

            if args.economic:
                torch.cuda.empty_cache()

            output, feat = model(student_input, requires_feat=requires_feat)

            loss_ori = args.ori_loss_weight * loss_fn(output, target)

            try:
                kd_loss_fn = model.module.kd_loss_fn
            except AttributeError:
                kd_loss_fn = model.kd_loss_fn
            loss_kd = args.kd_loss_weight * kd_loss_fn(z_s=output, z_t=output_t,
                                                       ...
                                                       index=index, contrastive_index=contrastive_index)
